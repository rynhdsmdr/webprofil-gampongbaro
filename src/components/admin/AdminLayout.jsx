  ...
  return (
    <div className="min-h-screen bg-slate-100 flex">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        {/* Top Navbar */}
        <header className="bg-white border-b border-slate-200 px-4 sm:px-6 py-3.5 flex items-center justify-between sticky top-0 z-30">
          <div className="flex items-center gap-3 min-w-0">
            <button
              onClick={() => setSidebarOpen(!sidebarOpen)}
              className="lg:hidden p-2 rounded-xl bg-slate-100 text-slate-700 hover:bg-slate-200 shrink-0"
              aria-label="Toggle admin sidebar"
            >
              {sidebarOpen ? <X className="w-5 h-5" /> : <Menu className="w-5 h-5" />}
            </button>
            <h1 className="font-serif font-bold text-slate-900 text-sm sm:text-xl truncate">
              Panel Admin Gampong Baro
            </h1>
          </div>
          ...
          <div className="flex items-center gap-2.5 shrink-0">
            ...
            <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-full bg-emerald-100 text-emerald-800 flex items-center justify-center font-bold text-xs border border-emerald-300">
              AG
            </div>
          </div>
        </header>
        ...
        {sidebarOpen && (
          <>
            ...
            />
            <aside className="fixed inset-y-0 left-0 z-50 w-72 bg-slate-900 text-slate-300 shadow-2xl flex flex-col lg:hidden animate-fadeIn">
              <div className="p-5 border-b border-slate-800 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-xl bg-emerald-600 flex items-center justify-center text-white font-bold text-sm shadow">
                    GB
                  </div>
                  <div>
                    <h2 className="font-serif font-bold text-white text-sm">Admin Panel</h2>
                    <p className="text-[10px] text-emerald-400">Gampong Baro</p>
                  </div>
                </div>
                <button
                  onClick={() => setSidebarOpen(false)}
              ...
              <div className="p-4 border-t border-slate-800 space-y-2">
                <button
                  onClick={() => { setSidebarOpen(false); handleResetData(); }}
                  className="flex items-center justify-center gap-2 w-full py-2 rounded-xl bg-slate-800 hover:bg-amber-900/40 text-amber-300 border border-slate-700 text-xs font-semibold transition"
                >
                  <RefreshCw className="w-3.5 h-3.5" /> Reset Data Default
                </button>
